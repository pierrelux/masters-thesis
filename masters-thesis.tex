...
The Markov property (or \textit{memoryless property}) could be explained in words by
saying that the \textit{future is independent of the past, given the present state}.
Therefore, knowing the value current value of $X_n = i$ is enough to characterize the
...
At first blush, it might be tempting to overlook the ramifications of graph theory
into that of \mdps. As it will be shown below, spectral graph theory provides a powerful
perspective for studying the dynamical properties of Markov chains. The graph
representation of Markov chain will act as a proxy so that matrix theory and linear
algebra will suffice to provide rich results in a concise manner. Some of the early
applications making use of this connection can be traced back to the
work of Simon and Ando in \cite{SimonAndo1961}.\todo{Talk about Fiedler 73}
...
as an initial foray towards a better theoretical grounding of the bottleneck concept to
which belongs this algorithm. The notion of random walk on an arbitrary graph
$\mathcal{G}$ will first be introduced, followed by a characterization of the convergence
time to a stationary distribution in terms of mixing time. It will be then shown how
...
Explain very intuitively what good options should be. They should be compressible, 
allow compression in the time dimension. Explain that through random walk, we
are just trying to analyze the transition structure associated with the optimal policy.
Fast mixing is just a proxy to what could be compression candidates. 

Puterman classification of MPD. Weakly communicating. We are looking for closed set of states with transient states under every policy.

Multichain two or more irreducible recurrent classes.

An MDP is weakly communicating if and only if there exists a randomized
stationary policy which induces a Markov chain with a single closed irreducible
class and a set of states which is transient under all stationary policies.

Terminology : \cite{Platzman1977}
Decomposition into ergodic subchain \cite{Fox1968}
\cite{Ross1991}
